...
// Divides the sanArrayIndex by 2 and rounds up to the nearest integer. This is the current move number.
function updateMoveNumber() {
  const moveNumberElement = document.getElementById('move-number');
  moveNumberElement.textContent = `Move Number: ${Math.ceil((sanArrayIndex / 2) + 1)}`;
  console.log(`Move Number: ${Math.ceil((sanArrayIndex / 2) + 1)}`);
}
