...
// Color the background of the board green for 3 seconds. This is called when the user has reached the end of the PGN string.
...
function pieceMove(move) {
  ...
  if (chessMove.san === sanArray[sanArrayIndex]) {
    ...
    console.log("success");
    sanArrayIndex++;
    window.setTimeout(makePGNMove, 250);
    updateGameInfo();
  ...
  // Not sure if this is all needed because we're not actually playing chess... just doing drills. Leaving it here for now.
  // if (chessMove !== null) {
